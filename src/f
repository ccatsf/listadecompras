...
// 🔥 MELHORADO: remove números e limpa melhor o nome
function extrairNomeProduto(url: string) {
  try {
    const partes = url.split("/");
    let ultimo = partes[partes.length - 1];

    ultimo = ultimo.split("?")[0]; // remove parâmetros
    ultimo = ultimo.replace(/\d+/g, ""); // remove números
...
export default function App() {
  ...
  const extractProductData = async (url: string) => {
    ...
    try {
      ...
      const imageUrl = `https://source.unsplash.com/200x200/?${encodeURIComponent(nomeBruto + ",product")}`;
