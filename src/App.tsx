...
function extrairNomeProduto(url: string) {
  try {
    ...
    ultimo = ultimo.split("?")[0];
    ultimo = ultimo.replace(/\d+/g, "");

    const nome = ultimo.replace(/-/g, " ").trim();

    return nome.length > 3 ? nome : "Produto";
  } catch {
    return "Produto";
  }
}
...
export default function App() {
  ...
  const extractProductData = async (url: string) => {
    ...
    try {
      const nomeBruto = extrairNomeProduto(url);
      const nome = formatarNome(nomeBruto);

      // 🔥 imagem melhor (tipo google)
      const imageUrl = `https://loremflickr.com/200/200/${encodeURIComponent(nomeBruto)}`;
      ...
      <header className="sticky top-0 z-10 bg-[#F2F2F7]/80 backdrop-blur-md px-4 pt-12 pb-4 border-b border-[#D1D1D6]">
        <div className="flex justify-between items-end">
          ...
          <button 
            ...
            className="w-10 h-10 bg-[#007AFF] rounded-full flex items-center justify-center text-white shadow-lg"
          >
            <Plus size={24} />
          </button>
        </div>
          ...
          <AnimatePresence>
            {products.map((product) => (
              <motion.div 
                ...
                animate={{ opacity: 1 }} 
                exit={{ opacity: 0 }} 
                className="bg-white rounded-2xl p-4 flex gap-4 shadow-sm border border-[#E5E5EA]"
              >
                ...
                <div className="flex-1">
                  ...
                  <input
                    type="number"
                    placeholder="Preço"
                    className="text-[#007AFF] font-bold bg-transparent outline-none w-28"
                    value={product.price}
                    onChange={(e) => {
          ...
          <div className="fixed inset-0 z-50 flex items-end justify-center">
            ...
            >
              ...
              <button 
                ...
                className="w-full bg-[#007AFF] text-white py-4 rounded-xl font-bold flex items-center justify-center gap-2"
              >
